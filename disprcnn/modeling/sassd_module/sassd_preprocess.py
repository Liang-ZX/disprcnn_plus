...
class SassdPreprocess:
    ...
    def back_project(self, depth_maps, mask_pred, targets, max_depth=160, fix_seed=False):
        ...
        for pt_per_roi in pt:
            pt_per_roi = pt_per_roi[:, ((pt_per_roi[0, :] > -40) & (pt_per_roi[0, :] < 40)).nonzero().reshape(-1)]
            pt_per_roi = pt_per_roi[:, ((pt_per_roi[1, :] > -40) & (pt_per_roi[1, :] < 40)).nonzero().reshape(-1)]
            pt_per_roi = pt_per_roi[:, ((pt_per_roi[2, :] < 80)).nonzero().reshape(-1)]
